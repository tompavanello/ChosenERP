...
func (a *App) handleUpdateMember(w http.ResponseWriter, r *http.Request) {
	...
	err := a.Store.WithTenant(r.Context(), b, func(tx pgx.Tx) error {
		var err error
		m, err = a.Members.Update(r.Context(), tx, id, in)
		return err
	})
	if err != nil {
...
func (a *App) handleCreateFamily(w http.ResponseWriter, r *http.Request) {
	...
	err := a.Store.WithTenant(r.Context(), b, func(tx pgx.Tx) error {
		var err error
		f, err = a.Families.Create(r.Context(), tx, claims.TenantID, claims.BranchID, in.Name)
		return err
	})
	if err != nil {
		writeErr(w, http.StatusInternalServerError, err.Error())
...
func (a *App) handleCreateVisitor(w http.ResponseWriter, r *http.Request) {
	...
	err := a.Store.WithTenant(r.Context(), b, func(tx pgx.Tx) error {
		var err error
		v, err = a.Visitors.Create(r.Context(), tx, claims.TenantID, claims.BranchID, in)
		return err
	})
	if err != nil {
		writeErr(w, http.StatusInternalServerError, err.Error())
