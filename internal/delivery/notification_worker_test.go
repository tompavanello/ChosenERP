...
func TestRenderTemplate(t *testing.T) {
	got := renderTemplate(
		"Ola, {primeiro_nome}! Feliz aniversario na {igreja}!",
		map[string]string{"primeiro_nome": "Ana", "igreja": "Igreja Central"},
	)
	want := "Ola, Ana! Feliz aniversario na Igreja Central!"
	if got != want {
...
func TestLoadLocation(t *testing.T) {
	if loc := loadLocation("America/Sao_Paulo"); loc == nil || loc.String() != "America/Sao_Paulo" {
		t.Errorf("loadLocation valido falhou: %v", loc)
	}
	if loc := loadLocation("TZ/Inexistente"); loc != time.UTC {
		t.Errorf("timezone invalido deve cair em UTC, veio %v", loc)
	}
...
func TestNotificationWorkerNowDefault(t *testing.T) {
	...
	if w.now().IsZero() {
		t.Error("now() sem override nao deveria ser zero")
	}
