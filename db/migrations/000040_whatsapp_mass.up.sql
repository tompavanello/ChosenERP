...
--   * #32 disparo segmentado: os filtros demograficos viajam no SendInput e sao
--     resolvidos em internal/announcements (nao exigem coluna nova).
--   * #31 automacoes (aniversario, lembrete de escala, boas-vindas ao visitante):
--     precisam de um outbox que aceite mensagens SEM `announcements` (o texto e
--     renderizado por destinatario) e de configuracao por tenant.
--
-- Por isso a fila de envio ganha snapshot de titulo/corpo, origem e uma chave de
-- deduplicacao (evita mandar o mesmo "feliz aniversario" duas vezes no dia).

-- ---------------------------------------------------------------------------
-- 1) Outbox: mensagens avulsas (automacoes) + snapshot + dedupe
-- ---------------------------------------------------------------------------
...
-- Uma automacao pode nao estar atrelada a um aviso do feed.
ALTER TABLE announcement_deliveries ALTER COLUMN announcement_id DROP NOT NULL;

-- Dedupe por tenant: o worker usa ON CONFLICT nesta chave para nao repetir.
CREATE UNIQUE INDEX idx_ann_dlv_dedupe
    ON announcement_deliveries (tenant_id, dedupe_key)
    WHERE dedupe_key IS NOT NULL;

-- ---------------------------------------------------------------------------
-- 2) Corrige a RLS do outbox: as politicas originais (000017) comparavam o
...
-- ---------------------------------------------------------------------------
-- 3) Configuracao das automacoes (uma linha por tenant)
-- ---------------------------------------------------------------------------
CREATE TABLE notification_settings (
    tenant_id                  uuid PRIMARY KEY REFERENCES tenants(id) ON DELETE CASCADE,
    birthdays_enabled          boolean NOT NULL DEFAULT false,
    birthday_template          text NOT NULL DEFAULT 'Feliz aniversario, {primeiro_nome}! A {igreja} celebra a sua vida hoje. Que Deus o(a) abencoe ricamente!',
    ...
    roster_template            text NOT NULL DEFAULT 'Ola, {primeiro_nome}! Lembrete: voce esta escalado(a) em "{titulo}" no dia {data}. Conte com voce!',
    ...
    visitor_welcome_template   text NOT NULL DEFAULT 'Ola, {primeiro_nome}! Foi uma alegria receber voce na {igreja}. Esperamos ver voce novamente em breve!',
